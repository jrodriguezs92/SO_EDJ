...
then
	...
	then
		...
		echo "  "
		echo -e '\e[36m>> type ./Install.sh'
		echo -e '\e[36mTo install with default ports'
		echo -e 'The default ports are:'
		echo -e '8001 -- Sequential server port'
		echo -e '8003 -- Forked server port'
		echo -e '8005 -- Threaded server port'
		echo -e '8007 -- Pre-Forked server port'
		echo -e '8009 -- Pre-Threaded server port \e[39m'
...
# If there is no arguments: by default map ports 8001, 8003, 8005, 8007, 8009
elif [ "$#" -eq 0 ]
then
	...
	# and with a defined volume.
	# REFERENCE: https://hub.docker.com/r/jrei/systemd-ubuntu
	sudo docker run -d -p 8009:8009 --privileged --name webserver_prethreaded -v /sys/fs/cgroup:/sys/fs/cgroup:ro webserver_prethreaded
...
# If there is a different number of arguments than 1, 0 or 5
else
